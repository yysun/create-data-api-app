...
`: `
  app.${method}('${path}', ${authentication}async (req, res) => {
    const body = req.body;
    const result = await model.${database.name}['${method} ${path}'](body);
    res.json(result);
  });
`}).join('');
